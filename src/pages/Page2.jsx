...
const Page2 = () => {
    useEffect(() => {
        ...
        headings.forEach((heading) => {
            const letters = heading.textContent.split('').map((letter) => `<span class="letter">${letter}</span>`).join('');
            heading.innerHTML = letters;

            const letterSpans = heading.querySelectorAll('.letter');
            gsap.set(letterSpans, { display: 'inline-block' });
            ...
            letterSpans.forEach((letter) => {
                gsap.fromTo(
                    ...
                    { y: 0 },
                    {
                        y: -10, // Adjust the movement upward
                        duration: 0.3,
                        ease: 'power1.out',
                        paused: true,
                        onHover: (el) => el.play(),
                ...
                heading.addEventListener('mouseenter', () => {
                    gsap.to(letterSpans, { y: -10, duration: 0.3, ease: 'power1.out', stagger: 0.05 });
                });
                heading.addEventListener('mouseleave', () => {
                    gsap.to(letterSpans, { y: 0, duration: 0.3, ease: 'power1.out', stagger: 0.05 });
                });
            });
        });
    }, []);
    ...
    return (
        <div id='section2' className=' text-black text-center p-[2vw]'>
            {/* <h3 className=' text-4xl font-[pf-4]'>© anzo.studio 2024 | designed and developed</h3> */}

            {['IMPACTFUL', 'DESIGN', 'IS', 'THE', 'DESIGN', 'THAT', 'WORKS'].map((text, index) => (
                <div key={index} className='heading rotateText mt-2 text-[12vw] text-black font-[pf-4] leading-[15vw]'>
                    {text}
                </div>
            ))}
            {/* <div className=' bg-emerald-200 flex items-center justify-center scale-75'>
